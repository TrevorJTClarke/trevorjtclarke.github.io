var TC = angular.module("tjtc", ["firebase"]);

TC.controller("Main", 
["$scope", "$firebase",    
function($scope, $firebase) {
    var DB = new Firebase("https://tjtc.firebaseio.com/blog");
    var weather = new Firebase("https://publicdata-weather.firebaseio.com/losangeles");
    var syncWS = $firebase(weather);
    var syncDB = $firebase(DB);

    $scope.weather = syncWS.$asObject();
    $scope.blogs = syncDB.$asArray();

    console.log($scope.weather);
    console.log($scope.blogs);
}]);

TC.filter('timeAgo', function (nowTime, timeAgo) {
...
.factory('timeAgo', function () {
  ...
  service.parse = function (iso8601) {
    if (angular.isNumber(iso8601)) {
      iso8601 = iso8601 + "";
      var originalTimestamp = iso8601.substring(0, iso8601.length - 1);
